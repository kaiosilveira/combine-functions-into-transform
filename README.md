...
Sometimes we see groups of functions repeatedly operating together over a chunk of data. These functions may be independent and well-defined, but their responsibilities are tightly related to some other, bigger aspect of a piece of computation. In these cases, we can create a higher-order transform function to express this bigger aspect and keep all clients consistent.

## Working example

The working example for this refactoring is a small system that calculates the taxable charge for... tea (?). It contains helper functions to calculate the `baseRate` and the `taxThreshold`, but there is a lot of duplication when it comes to calculating the base charge inside the clients of this code. Our goal is to combine these different aspects into a transform function that takes care of all the details and returns an enhanced object.

The starting code for this example contains a `tax-utils` module with the utility functions mentioned above (`baseRate` and `taxThreshold`), it also contains the code for the clients themselves:
...
After finishing this refactoring, the code will look quite different, but the core part is the new `enrichReading`.
...
To perform this refactoring, we first introduce the `enrichReading` function, which will be responsible for adding the new fields we want to the `reading` object. To start with, this function does nothing but create and return a clone of the original `reading` input object:
...
And update `client3.js` to use the `baseCharge` field from the enriched `reading` object:
